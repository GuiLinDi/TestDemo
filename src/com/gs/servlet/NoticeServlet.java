...
public class NoticeServlet extends HttpServlet {
	...
	private void AU_2in1(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("notice/AU_2in1.jsp").forward(request, response);
	}
	...
	private void select(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		util.setUrl("/NoticeServlet?param=pageSelect&type=1");
		NoticeDao dao  = new NoticeDao();
		List list = dao.pageSelect(title,type, util.getBeg(), util.getPageSize());
		request.setAttribute("datalist", list);
		request.setAttribute("util", util);
		request.getRequestDispatcher("notice/select.jsp").forward(request, response);
