...
<table height="30" cellspacing="0" cellpadding="0" width="98%" 
      ...
      <tbody>
         ...
         <tr>
    <td class="wz_left">&nbsp;</td>
    <td nowrap="nowrap" class="wz_middle" >工作日志管理</td>
  ...
  <div id="top">
    <table  width="98%" border="0" align="center" cellpadding="4" cellspacing="1" class="datatable" >
      <tr>
        ...
        <tr>
          <td  colspan="6"> 
        <input type="button" value="查询"  class="bt" onClick="doSubmit('<%=path %>/JoblogServlet?param=select')" />
        <input type="button"  value="增加"  class="bt" onClick="doSubmit('<%=path %>/JoblogServlet?param=au&Joblog_id=')"/>
      </td>
      </tr>
    </table>
  </div>
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="1" class="datatable1" >
          <tr>
			...
			<th width="8%" class="listTableHead">修改时间</th>
		...
		<tr>
            ...
            <td class="listTableLine1" >&nbsp;${jl.rectime }</td>
            <td class="listTableLine1" >&nbsp;${jl.type }</td>
			<td class="listTableLine1" >
			<a  style="cursor:pointer;" href="<%=path %>/JoblogServlet?param=delete&Joblog_id=${jl.id }&usercode=${jl.usercode}"><u>删除</u></a>
			<a  style="cursor:pointer;" onClick="doSubmit('<%=path %>/JoblogServlet?param=au&usercode=${jl.usercode}&Joblog_id=${jl.id }')"><u>修改</u></a>
			</td>
          </tr>
          </c:forEach>
</table>
